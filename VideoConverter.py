import subprocess, time, sys, os, tool, json, configparser, requests, logging
from threading import Thread, Lock
from PyQt6 import QtWidgets, QtCore, QtGui, uic, QtQml
from PyQt6.QtWidgets import QMessageBox, QFileDialog, QApplication
from PyQt6.QtCore import QRunnable, pyqtSlot, QThreadPool

logging.basicConfig(level=logging.INFO, filename="Logs\\Update.log",filemode="a+",
                    format="[%(asctime)s] %(levelname)s: %(message)s")

def LoadCfg():
    global Config, THREADS, HOME_FOLDER, SAVE_FOLDER, LANGUAGE, Speed, Tune, Codec, Langs, config, Check_ver_link, Version
    ...
    Codec = config["Codecs"]
    Langs = config["Langs"]
    Check_ver_link = config["Update"]["check_url"]
...
def check_for_updates():
    logging.info("Looking for updates")
    for i in range(3):
        try:
            logging.info("Trying to connect to update server...")
            r = requests.get(Check_ver_link,timeout=0.5)
            if r.json()["version"] != Version:
                logging.info("Found an update!")
                MainWindow.Ask_for_update(MainWindow,r.json()["version"])
                return
            else: 
                logging.info("Update not founded")
                return
        except requests.exceptions.RequestException:
            logging.error(f"Connection failed. #{i} try")
...
class Settings(QtWidgets.QWidget):
    ...
    def retranslateUi(self):
        ...
        self.setWindowTitle(_translate("Form", "Settings"))
...
class MainWindow(QtWidgets.QMainWindow): #Main Window
    ...
    def ChangeLanguage(self, lang):
        ...
        else: QtWidgets.QApplication.removeTranslator(self.trans)
        self.retranslateUi()
    ...
    def Ask_for_update(self, ver):
        ...
        icon.addPixmap(QtGui.QPixmap("Vid.ico"), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        msgBox.setWindowIcon(icon)
        msgBox.setText(f"New version available!\n{ver}\nInstall now?")
        ...
        if returnValue == QMessageBox.StandardButton.Ok:
            ...
            subprocess.Popen(["Update.exe", f"{ver}", "-backup"], creationflags=DETACHED_PROCESS, close_fds=True, shell=True)
            exit()
    
    def retranslateUi(self):
        self.setWindowTitle(QtCore.QCoreApplication.translate("MainWindow", "Ultimate Video Converter"))
